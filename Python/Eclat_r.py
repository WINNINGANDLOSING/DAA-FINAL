...
# Data Preprocessing
#
dataset = pd.read_csv('contextPasquier99.csv', header = None)
...
transact_list = []
for i in range(0, 5):
  #transact_list.append([str(dataset.values[i,j]) for j in range(0, 20)]) # add the transactions to the above created
  transact_list.append([str(dataset.values[i,j]) for j in range(0,4)])
...
def eclat(transactions_list, output_filename, min_support = 0.002):
    ...
    for combo in combos_TO_counts.keys():
        ...
        #   i.e. M's popularity
        support = float(combos_TO_counts[combo])/len(transact_list)
...
combos_vs_supports = eclat(transact_list, "./eclat.tsv", min_support = 0.4)
